...
def convert_JSON_to_locations_dictionary(scene_graph):
    ...
    if "room" in scene_graph.keys():
        ...
        # Scene graph is already a location -> [object,...] mapping
        for loc, objs in scene_graph.items():
            for obj_id in objs:
                # obj_id already contains class and id (e.g. "fork_40")
                locations[str(obj_id)] = str(loc)
...
def extract_parameters_from_action(action):
...
def simpler_grounding(
    ...
    for subplan in subplans:
        ...
        for action in subplan["actions"]:
            ...
            action_parameters = extract_parameters_from_action(action)
...
def verify_groundability_in_scene_graph(
    ...
    for subplan in subplans:
        ...
        if not successful_actions:
            ...
            grounding_percentage = total_successful_actions * 1.0 / len(plan)

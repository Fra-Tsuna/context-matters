...
def plan_with_output(domain_file_path, problem_dir, plan_file_path, env=None, search_flag='--search "astar(ff())"', timeout=180):
...
def initialize_pddl_environment(domain_file_path, problem_dir):
    # Create PDDLEnv
    env = PDDLEnv(domain_file_path, problem_dir, operators_as_actions = True, dynamic_action_space=True)

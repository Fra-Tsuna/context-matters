...
def generate_domain(
    ...
    answer = agent.llm_call(prompt, question)
    ...
    domain_pddl = answer.replace("`", "").replace("pddl", "").replace("lisp", "")
...
def determine_problem_possibility(
    ...
    # Call the LLM
    response = agent.llm_call(prompt, question)
...
def generate_problem(
    ...
    system_prompt += """
        ...
        IGNORE ROOM CONNECTIONS, YOU CAN MOVE FROM ANY ROOM TO ANY ROOM, WITHOUT THE NEED FOR A PREDICATE "connected". NEVER USE THIS KIND OF PREDICATE.
    ...
    problem = agent.llm_call(system_prompt, user_prompt)
...
def refine_problem(
    ...
    reason_of_failure = agent.llm_call(reason_system_prompt, reason_user_prompt)
